...
public class NearbyEntityTracker<T extends LivingEntity> implements NearbyEntityListener {
    ...
    private T getFirst(T entity1, T entity2) {
        if (this.getEntityClass() == Player.class) {
            ...
            List<? extends Player> players = this.getEntity().getCommandSenderWorld().players();
    ...
    public String toString() {
        return super.toString() + " for entity class: "
                + this.clazz.getName() + ", around entity: "
                + this.getEntity().toString() + " with NBT: "
                + this.getEntity().saveWithoutId(new CompoundTag());
    }
