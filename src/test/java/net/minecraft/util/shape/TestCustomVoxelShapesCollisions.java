...
public class TestCustomVoxelShapesCollisions {
    ...
    public static AABB getRandomBox(Random random) {
        double x1 = random.nextDouble() * 2;
        double y1 = random.nextDouble() * 2;
        double z1 = random.nextDouble() * 2;
        return new AABB(x1, y1, z1, 2 * x1 + random.nextGaussian(), 2 * y1 + random.nextGaussian(), 2 * z1 * random.nextGaussian());
    }

    public static VoxelShape[] getVanillaModdedVoxelShapePair(AABB box) {
        return new VoxelShape[]{Shapes.cuboid(box), cuboid(box)};
    }

    private static VoxelShape cuboid(AABB box) {
    ...
    public void testCollisions() {
        ...
        this.boxes = new AABB[20];
        this.boxes[0] = new AABB(0.2931994021407849, 0.5175531777466607, 0.14575020167685837, 1.4562191976519117, 2.2389429614133496, 0.31827209851790766);
        this.boxes[1] = new AABB(0.5 - 0.5E-7, 0.75, 0.125 + 1E-7, 1, 1 + 1E-6, 0.5);
        for (int i = 2; i < 20; i++) {
                    ...
                    for (int y2 = y + 1; y2 <= 8; y2++) {
                        for (int z = 0; z <= 7; z++) {
                            for (int z2 = z + 1; z2 <= 8; z2++) {
                                VoxelShape[] pair = getVanillaModdedVoxelShapePair(new AABB(x / 8D, y / 8D, z / 8D, x2 / 8D, y2 / 8D, z2 / 8D));
                                ...
                                pair = getVanillaModdedVoxelShapePair(new AABB(x / 8D + 2E-7 * this.rand.nextDouble(), y / 8D + 4E-8, z / 8D, x2 / 8D, y2 / 8D + 9E-8, z2 / 8D));
                                this.testShapeBehaviorEquality(pair);
                            }
                        }
                    }
